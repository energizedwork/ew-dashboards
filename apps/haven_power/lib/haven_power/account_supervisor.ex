defmodule HavenPower.AccountSupervisor do
  @moduledoc """
  Supervisor to handle the creation of dynamic `HavenPower.Account` processes using a
  ...
  `HavenPower.Account.start_link` function.
  ...
  is attached to a running `HavenPower.Account` process.

  If the `account_id` is not present in the registry, it will create a new `HavenPower.Account`
  ...
  @doc """
  Determines if a `HavenPower.Account` process exists, based on the `account_id` provided.

  Returns a boolean.

  ## Example
      iex> HavenPower.AccountSupervisor.account_process_exists?(6)
      false
  """
  ...
  @doc """
  Returns the count of `HavenPower.Account` processes managed by this supervisor.

  ## Example
      iex> HavenPower.AccountSupervisor.account_process_count
      0
  """
  ...
  def get_all_account_widgets_ordered do
    account_ids() |> Enum.map(&(%{ account_id: &1, widgets_sold: HavenPower.Account.widgets_ordered(&1) }))
  end
  ...
  def init(_) do
    children = [
      worker(HavenPower.Account, [], restart: :temporary)
    ]
